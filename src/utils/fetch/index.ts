...
async function fetchCore({
  ...
  if (params && Object.keys(params).length > 0) {
    const urlParams = Object.entries(params)
      .map(([key, value]) => {
        const generatePropertyQueryString = (key: string | number | boolean, value: string | number | boolean) => `${encodeURIComponent(key)}=${encodeURIComponent(value)}`;

        if (Array.isArray(value)) {
          return value.map((valueOfArray) => generatePropertyQueryString(key, valueOfArray)).join('&');
        }
